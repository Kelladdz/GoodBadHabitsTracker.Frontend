...
const Calendar = React.forwardRef(({props, withoutArrows, cellSize, headerPadding, type}, ref) => {
    const { currentDate, startDate, today, numberOfLastDayOfCurrentMonth, firstDayOnCalendar, calendarDays, calendarRows, previousMonth, nextMonth, handleDayClick } = useCalendar(type, cellSize);
    const currentDateString = currentDate.toISOString().substring(0, 10);
    const [activeDivSprings, activeDivApi] = useSpring(() => ({ left: '0.5rem', top: '0rem', config: { duration: 100 } }));

    useEffect(() => {
        if (calendarDays && calendarDays.some(day => day === startDate)) {
        ...
        <div ref={ref} className={styles.calendar}>
            ...
            <div style={{gridTemplateRows: `repeat(${calendarRows}, ${cellSize})`, gridTemplateColumns: `repeat(7, ${cellSize})`}} className={styles['days-of-month']}>
                {calendarDays && calendarDays.map((day, index) => {
                    ...
                    if (new Date(day) >= new Date(today.getFullYear(), today.getMonth(), today.getDate())) {
                        handleDayClick(day)
                    }}}>
                        <span key={index} className={styles['day-of-month']}>{day.split('-')[2]}</span>
                    </div>})}
                {type === CALENDAR_TYPES.form && calendarDays && calendarDays.some(day => day === startDate) && <animated.div style={activeDivSprings} className={styles['floating-circle']}></animated.div>}
            </div>
        </div>  
    )
});
